# Analysis of three-cohort LUAD & LUSC real data - incongruous ranking between z-score ranking and lfdr ranking (kernel & repfdr)
...
here::i_am("Lung/three_incon_adeno_scc.R")
...
lfdrnew    <- fread(paste0(outputDir,"/rep_three_adeno_aID",aID, "_newlfdr.txt"))
...
write.table(
  res_all,
  paste0(fnameRoot, "_three_incon_adeno.txt"),
  append = FALSE,
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE,
...
#SCC
fnameRoot <- paste0(outputDir, "/three_rep_scc_data_aID", aID)
# where is the data
summaryStatDir <- here::here("Data")
...
lfdrKernel <- fread(paste0(fnameRoot, "_newResKernel.txt"))
lfdrRepfdr <- fread(paste0(fnameRoot, "_newResRepfdr.txt"))
lfdrnew    <- fread(paste0(outputDir,"/rep_three_scc_aID",aID, "_newlfdr.txt"))
...
write.table(
  res_all,
  paste0(fnameRoot, "_three_incon_scc.txt"),
  append = FALSE,
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE,
