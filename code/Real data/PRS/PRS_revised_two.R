...
tempNew <- fread(file.path(outputDir, "two_rep_overall_aID2_newlfdr.txt"),
...
N_ILCCO <- calc_neff(29266, 56450); N_MVP <- calc_neff(10398, 62708)
w1 <- sqrt(N_ILCCO); w2 <- sqrt(N_MVP)

meta_SNPs <- three_Z %>%
  mutate(Z_meta = (zILCCO * w1 + zMVP * w2) / sqrt(w1^2 + w2^2),
...
#   fwrite(s$snps, file.path(geno_dir, paste0("base_", s$tag, ".txt")),
#          sep = "\t", quote = FALSE)
# }
...
#                    file.path(geno_dir, sprintf("%s_ukb_chr%s_filtered", s$tag, chr)),
#                    white_id$eid)
#   }
# }
...
combine_chr_scores <- function(tag,
                               pattern_template = "^%s_prs_chr[0-9]+_r2\\.all_score$") {
...
fwrite(summary_tab, file.path(outputDir_Fig, "two_way_AUC_summary_cv.csv"))

# DeLong: median p across reps + fraction of reps with p < 0.05
# Replace the two apply() lines with:
...
fwrite(or_df,   file.path(outputDir_Fig, "two_way_decile_OR_cv.csv"))
