#make a three way Z statistics for adenocarcinoma Lung Cancer
setwd("csmGmm_reproduce/Lung")
here::i_am("Lung/clean_adeno.R")
...
#Read UKB
rawUKB <- fread(paste0(dataDir,"/lung_cancer_summary_statistics/UKB/full_adeno_maf00005_regenie.txt"))
rawUKB <- rawUKB %>% 
...
#Read ILCCO 
rawILCCO <- fread(paste0(dataDir,"/lung_cancer_summary_statistics/ILCCO/LC_Adeno_hg19_chr",1,".txt")) 


for (i in 2:22) {
  temp <- fread(paste0(dataDir,"/lung_cancer_summary_statistics/ILCCO/LC_Adeno_hg19_chr",i,".txt"))
...
#read mvp
rawMVP <- fread(paste0(dataDir,"/lung_cancer_summary_statistics/MVP/LUAD-MVP-EA.tsv.gz"))
cleanMVP <- rawMVP %>%
...
write.table(final, paste0(dataDir, "/lc_adeno_three.txt"), append=F, quote=F, row.names=F, col.names=T)
